...
class DBHelper{
  ...
  insertDataIntoTable(Quote data) async {
    db = await initDB();
    categoryTable = "${query.toLowerCase().replaceAll(' ', '_')}table";
    String sqlQuery =  "INSERT INTO $categoryTable($colQuote,$colAuthor, $colFavorite,$colMarkedAsRead,$colBGImage) VALUES(?,?,?,?,?);";
    List args = [data.quote,data.author,data.favorite,data.markedAsRead,data.backgroundImage];
